...
LCDWIKI_SPI::LCDWIKI_SPI(uint16_t model, int8_t cs, int8_t cd, int8_t miso,
...
LCDWIKI_SPI::LCDWIKI_SPI(int16_t wid, int16_t heg, int8_t cs, int8_t cd,
                         int8_t miso, int8_t mosi, int8_t reset, int8_t clk,
                         int8_t led) {
  _cs = cs;
  _cd = cd;
  _miso = miso;
...
void LCDWIKI_SPI::Set_Addr_Window(int16_t x1, int16_t y1, int16_t x2,
                                  int16_t y2) {
  CS_ACTIVE;
...
void LCDWIKI_SPI::Fill_Rect(int16_t x, int16_t y, int16_t w, int16_t h,
                            uint16_t color) {
  int16_t end;
  if (w < 0) {
    w = -w;
...
// get lcd width
int16_t LCDWIKI_SPI::Get_Width(void) const { return width; }

// get lcd height
int16_t LCDWIKI_SPI::Get_Height(void) const { return height; }
  ...
  if ((lcd_driver == ID_932X) || (lcd_driver == ID_9225)) {
    ...
    switch (rotation) {
      case 0:
        ...
        break;
      case 1:
        val = ILI9341_MADCTL_MX | ILI9341_MADCTL_MV | ILI9341_MADCTL_ML |
              ILI9341_MADCTL_BGR;  // 90 degree
        break;
      case 2:
        val = ILI9341_MADCTL_MY | ILI9341_MADCTL_MX |
              ILI9341_MADCTL_BGR;  // 180 degree
        break;
      case 3:
        val = ILI9341_MADCTL_MY | ILI9341_MADCTL_MV |
              ILI9341_MADCTL_BGR;  // 270 degree
        break;
    }
    writeCmdData8(MD, val);
  }
    ...
    switch (rotation) {
      case 0:
        val = ILI9341_MADCTL_MX | ILI9341_MADCTL_MY |
              ILI9341_MADCTL_BGR;  // 0 degree
        break;
      case 1:
        val = ILI9341_MADCTL_MV | ILI9341_MADCTL_MY |
              ILI9341_MADCTL_BGR;  // 90 degree
        break;
      case 2:
        ...
        break;
      case 3:
        val = ILI9341_MADCTL_MX | ILI9341_MADCTL_ML | ILI9341_MADCTL_MV |
              ILI9341_MADCTL_BGR;  // 270 degree
        break;
    }
    writeCmdData8(MD, val);
    ...
    switch (rotation) {
      ...
      case 1:
        ...
        break;
      case 2:
        val = ILI9341_MADCTL_MY | ILI9341_MADCTL_ML |
              ILI9341_MADCTL_BGR;  // 180 degree
        break;
      case 3:
        val = ILI9341_MADCTL_MX | ILI9341_MADCTL_MY | ILI9341_MADCTL_ML |
              ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR;  // 270 degree
        break;
    }
    writeCmdData8(MD, val);
  }
...
// 2  :  180 degree
// 3  :  270 degree
uint8_t LCDWIKI_SPI::Get_Rotation(void) const { return rotation; }
...
void LCDWIKI_SPI::SH1106_Draw_Bitmap(uint8_t x, uint8_t y, uint8_t width,
                                     uint8_t height, uint8_t *BMP,
                                     uint8_t mode) {
  uint8_t i, j, k;
  uint8_t tmp;
...
void LCDWIKI_SPI::start(uint16_t ID) {
  ...
  switch(ID) {
    ...
    case 0x9341:
      ...
      // WIDTH = 240,HEIGHT = 320;
      // width = WIDTH, height = HEIGHT;
      XC = ILI9341_COLADDRSET, YC = ILI9341_PAGEADDRSET,
      CC = ILI9341_MEMORYWRITE, RC = HX8357_RAMRD, SC1 = 0x33, SC2 = 0x37,
      MD = ILI9341_MADCTL, VL = 0, R24BIT = 1;
